...
data_recency['recencyCluster'] = kmeans.predict(data_recency[['recency']])


#Oclusters
def purchase_cluster(group_label, label,dataframe,ascending):
...
data_recency = purchase_cluster('recencyCluster', 'recency',data_recency,False)
print(data_recency.head())
print(data_recency.recency.describe())
...
fig = go.Figure(
    data=go.Histogram(x=data_recency.query('frequency < 730')['frequency'],
        marker=dict(
        color = "lightgray",
        line = dict(color = "darkgray",
        width = 2)))
...
data_recency = purchase_cluster('frequencyCluster', 'frequency',data_recency,True)
print(data_recency.groupby('frequencyCluster')['frequency'].describe())
#Print Recency Cluster
data_recency = purchase_cluster('recencyCluster', 'recency',data_recency,True)
print(data_recency.groupby('recencyCluster')['recency'].describe())
...
data_recency = purchase_cluster('monetaryValueCluster', 'monetaryValue',data_recency,True)
print(data_recency.groupby('monetaryValueCluster')['monetaryValue'].describe())

#Overall Score
data_recency['total'] = data_recency['recencyCluster'] + data_recency['frequencyCluster'] + data_recency['monetaryValueCluster']
print(data_recency.groupby('total')['recency','frequency','monetaryValue'].mean())
